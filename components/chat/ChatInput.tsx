...
// const base_url =  process.env.NODE_ENV === 'production' ? '/sdmp/claudable' : '/sdmp/claudable';
...
export default function ChatInput({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if ((message.trim() || uploadedImages.length > 0) && !disabled) {
      // If only images without text, use a placeholder message for the AI
      const finalMessage = message.trim() || (uploadedImages.length > 0 ? 'Please analyze the uploaded image(s).' : '');
  ...
  const handleFiles = async (files: FileList) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        // Use API URL instead of blob URL for persistent image access
        const imageUrl = `${API_BASE}/api/assets/${projectId}/${result.filename}`;
          ...
          <div className="flex items-center gap-2">
            ...
            {preferredCli && (
              <div className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded-full">
                ...
                />
                <span>
                  {preferredCli === 'agent' ? 'Claude Agent' : preferredCli}
                </span>
                {selectedModel && (
                  <>
