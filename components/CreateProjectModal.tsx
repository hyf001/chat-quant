...
const CLI_OPTIONS: CLIOption[] = [
  {
    ...
    name: 'Claude Agent',
    icon: '🤖',
    description: 'Anthropic Claude Agent with agentic capabilities',
    color: 'from-orange-500 to-red-600',
    downloadUrl: 'https://github.com/anthropics/claude-code',
    installCommand: 'npm install -g @anthropic-ai/claude-code',
    models: [
      { id: 'claude-sonnet-4-5', name: 'Claude Sonnet 4.5', description: 'Latest model with superior coding capabilities', supportsImages: true },
    ...
    features: ['Agentic reasoning', 'Code generation', '1M context window']
  }
];
...
export default function CreateProjectModal({ open, onClose, onCreated, onOpenGlobalSettings }: CreateProjectModalProps) {
  ...
  const [selectedCLI, setSelectedCLI] = useState<string>('agent');
  ...
  const loadGlobalSettings = async () => {
    try {
      ...
      if (settings) {
        ...
        const defaultCLI = settings.default_cli || 'agent';
        ...
        if (availableDefaultCLI) {
          ...
          setSelectedCLI('agent');
        }
        
        setFallbackEnabled(settings.fallback_enabled ?? true);
        
        // Set default model for the selected CLI
        const selectedCLIId = availableDefaultCLI ? defaultCLI : (enabled[0]?.id || 'agent');
        ...
        const firstCLI = available[0]?.id || 'agent';
  ...
  const handleInitializationComplete = (projectId: string) => {
    ...
    if (globalSettings) {
      setSelectedCLI(globalSettings.default_cli || 'agent');
      setFallbackEnabled(globalSettings.fallback_enabled ?? true);
      const cliSettings = globalSettings.cli_settings?.[globalSettings.default_cli || 'agent'];
  ...
  async function submit() {
    ...
    if (useDefaultSettings && globalSettings) {
      finalCLI = globalSettings.default_cli || 'agent';
