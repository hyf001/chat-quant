...
// Python executable names to try (prioritize Python 3.12)
...
function setupWithUv() {
  ...
  if (!fs.existsSync(venvDir)) {
    ...
    try {
      // Try to use Python 3.12 with uv
      execSync('uv venv .venv --python python3.12', { cwd: apiDir, stdio: 'inherit' });
      console.log('  Virtual environment created with Python 3.12');
    } catch (error) {
      // Fallback to default Python if 3.12 not found
      console.log('  Python 3.12 not found, trying default Python...');
      try {
...
function setupWithPip() {
  ...
  try {
    execSync(`"${pipPath}" install --upgrade pip -i https://pypi.tuna.tsinghua.edu.cn/simple`, { cwd: apiDir, stdio: 'ignore', timeout: 60000 });
  } catch {
  ...
  try {
    execSync(`"${pipPath}" install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple`, { cwd: apiDir, stdio: 'ignore', timeout: 180000 });
    console.log('  Python packages installed');
  } catch (error) {
    console.error('\nFailed to install Python packages');
    console.error('\nHow to fix:');
    console.error('   Option 1: Install manually with Chinese mirror');
    if (isWindows) {
      console.error('      cd apps/api');
      console.error('      .venv\\Scripts\\pip install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple');
    } else {
      console.error('      cd apps/api');
      console.error('      source .venv/bin/activate');
      console.error('      pip install -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple');
    }
