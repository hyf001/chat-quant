...
type SettingsTab = 'general' | 'ai-assistant' | 'environment';
...
export function ProjectSettings({
  ...
  const tabs: { id: SettingsTab; label: string; icon: React.ReactElement }[] = [
    { id: 'general', label: '通用', icon: <span className="w-4 h-4 inline-flex"><FaCog /></span> },
    { id: 'ai-assistant', label: 'AI助手', icon: <span className="w-4 h-4 inline-flex"><FaRobot /></span> },
    { id: 'environment', label: '环境变量', icon: <span className="w-4 h-4 inline-flex"><FaLock /></span> },
  ];
