...
import { getFileIconConfig } from '../scripts/utils';
...
interface ChatLogProps {
  ...
  logClickFile?: (fileInfo: fileEntry) => void;
  startRequest?: (requestId: string) => void;
  completeRequest?: (requestId: string, isSuccessful: boolean, errorMessage?: string) => void;
}

export default function ChatLog({ projectId, onSessionStatusChange, onProjectStatusUpdate, startRequest, completeRequest, logClickFile}: ChatLogProps) {
            ...
            components={{
              p: ({ children }) => {
                ...
                const hasPlanning = childrenArray.some(child => {
                  if (typeof child === 'string' && child.includes('规划下一步行动...')) {
                    return true;
                  }
                  return false;
                });
                ...
                >
                  {message.role === 'user' ? (
                    ...
                    <div className="flex justify-end">
                      <div className="max-w-[90%] bg-gray-100 dark:bg-gray-800 rounded-lg px-4 py-3">
                                    ...
                                    return (
                                      <div className="mt-2 flex flex-wrap gap-2 max-w-full">
                                        {attachments.map((attachment: any, idx: number) => {
                                          const fileUrl = `${API_BASE}${attachment.url}`;
                                          ...
                                          if (isImage) {
                                            return (
                                              <div key={idx} className="relative group max-w-full cursor-pointer" onClick={() => logClickFile?.(attachment)}>
                                            ...
                                            // 如果是其他文件,显示文件名和文件图标
                                            const iconConfig = getFileIconConfig(fileExt);

                                            return (
                                              <div key={idx} className="relative group w-[100%] cursor-pointer" onClick={() => logClickFile?.(attachment)}>
                                                <a className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg px-3 py-2 border border-gray-300 dark:border-gray-600 transition-colors">
                                                  <svg className={`w-5 h-5 ${iconConfig.colorClass} flex-shrink-0`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
