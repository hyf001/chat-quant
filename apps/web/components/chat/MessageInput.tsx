...
export function MessageInput({ 
  ...
  const handleSend = () => {
    if (content.trim() || uploadedImages.length > 0) {
      ...
      const finalContent = content.trim() || (uploadedImages.length > 0 ? 'Please analyze the uploaded image(s).' : '');
      onSend(finalContent, uploadedImages);
      setContent('');
      setUploadedImages([]);
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={handleSend}
          disabled={disabled || (!content.trim() && (!uploadedImages || uploadedImages.length === 0))}
