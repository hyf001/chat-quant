...
import { assistantOptions,assistantBrandColors,modelsByAssistant} from '../scripts/utils';
...
export default function HomePage() {
  ...
  const [selectedAssistant, setSelectedAssistant] = useState('analysis');
  ...
  useEffect(() => {
    ...
    const cli = globalSettings?.default_cli || 'analysis';
    ...
    if (modelFromGlobal) {
      ...
      // Fallback per CLI
      if (cli === 'analysis') setSelectedModel('claude-sonnet-4-5');
    }
  ...
  // Clear loading state when returning to page
  useEffect(() => {
    // Clear loading state when component mounts (user returned to page)
    setLoadingProjectId(null);
  }, []);
  const [showAssistantDropdown, setShowAssistantDropdown] = useState(false);
  const [showModelDropdown, setShowModelDropdown] = useState(false);
  const [isCreatingProject, setIsCreatingProject] = useState(false);
  const [uploadedImages, setUploadedImages] = useState<{ id: string; name: string; url: string; path: string; file?: File; file_type?: 'image' | 'csv' | 'text' | 'unknown' }[]>([]);
        ...
        if (response.ok) {
          ...
          assistantOptions.forEach(cli => {
            fallbackStatus[cli.id] = {
              installed: cli.id === 'claude' || cli.id === 'agent', // Default installed for known CLIs
              checking: false,
  ...
  const formatCliInfo = (cli?: string, model?: string) => {
    const cliName = cli === 'agent' ? 'Claude Agent' : cli || 'Unknown';
  ...
  async function updateProject(projectId: string, newName: string) {
    try {
      ...
      if (response.ok) {
        showToast('项目名称已成功更新', 'success');
        await load();
        setEditingProject(null);
      } else {
        const errorData = await response.json().catch(() => ({ detail: '更新项目名称失败' }));
        showToast(errorData.detail || '更新项目名称失败', 'error');
      }
    } catch (error) {
      showToast('更新项目名称失败，请重试', 'error');
    }
  }
  ...
  const handleFiles = async (files: FileList) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        // Determine file type
        let fileType: 'image' | 'csv' | 'text' | 'unknown' = 'unknown';
        if (isImage) fileType = 'image';
        else if (isCsv) fileType = 'csv';
        else if (isTxt) fileType = 'text';
  ...
  const handleSubmit = async () => {
    if ((!prompt.trim() && (!uploadedImages || uploadedImages.length === 0)) || isCreatingProject) return;
      ...
      if (finalPrompt.trim()) {
        try {
          ...
          if (actResponse.ok) {
            ...
            const res = await actResponse.json()
    ...
    <div className="flex h-screen relative overflow-hidden bg-white dark:bg-black">
      ...
      <div className="absolute inset-0">
        ...
        <div 
          ...
          style={{
            background: `radial-gradient(circle at 50% 100%, 
              ${assistantBrandColors[selectedAssistant]}60 0%, 
              ${assistantBrandColors[selectedAssistant]}3D 20%, 
              ${assistantBrandColors[selectedAssistant]}30 50%, 
              transparent 70%)`
          }}
        />
        ...
        <div 
          ...
          style={{
            background: `radial-gradient(circle at 50% 100%, 
              ${assistantBrandColors[selectedAssistant]}30 0%, 
              ${assistantBrandColors[selectedAssistant]}20 20%, 
              transparent 50%)`
          }}
        />
      </div>
                        ...
                        <input
                          ...
                          onBlur={(e) => {
                            ...
                            setTimeout(() => {
                              ...
                              if (!activeElement || !currentForm || !currentForm.contains(activeElement)) {
                                setEditingProject(null);
                              }
                            }, 200);
                          }}
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            {project.preferred_cli && (
                              <div className="flex items-center gap-1">
                                ...
                                >
                                  {project.preferred_cli === 'agent' ? 'Claude Agent' :
                                   project.preferred_cli}
                                </span>
                              </div>
                            )}
          ...
          <div className="w-full max-w-4xl">
            <div className="text-center mb-12">
              <div className="flex justify-center mb-6">
                <img src="./logoIcon.png" style={{height: '72px', marginRight: '10px'}}></img>
                <h1 
                ...
                >
                  {assistantOptions.find(d => d.id === selectedAssistant)?.name}Agent
                </h1>
              </div>
              ...
              <div className="mb-4 flex flex-wrap gap-2">
                {uploadedImages.map((file, index) => (
                  <div key={file.id} className="relative group">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 text-white text-xs px-1 py-0.5 rounded-b-lg truncate" title={file.name}>
                      {file.file_type === 'image' ? `Image #${index + 1}` : file.name}
                    </div>
                    <button
                      type="button"
            ...
            >
              ...
              <div className="flex gap-1 flex-wrap items-center">
                ...
                <div className="relative z-[200]" ref={assistantDropdownRef}>
                  ...
                  >
                    <span className="hidden md:flex text-sm font-medium">
                      {
                        assistantOptions.find(d => d.id === selectedAssistant)?.name
                      }
                    </span>
                  ...
                  {showAssistantDropdown && (
                    <div className="absolute top-full mt-1 left-0 z-[300] min-w-full whitespace-nowrap rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-gray-900 backdrop-blur-xl shadow-lg">
                      {assistantOptions.map((option) => (
                        ...
                        >
                          {
                            option.icon && <div className="w-4 h-4 rounded overflow-hidden">
                              <img 
                                src={option.icon} 
                                alt={option.name}
                ...
                {/* <div className="relative z-[200]" ref={modelDropdownRef}>
                  ...
                  >
                    <span className="text-sm font-medium whitespace-nowrap">{(() => {
                      ...
                      if (!found) {
                        if ((selectedAssistant === 'claude' || selectedAssistant === 'agent') && selectedModel === 'claude-sonnet-4-5') {
                          return 'Claude Sonnet 4.5';
                        } else if ((selectedAssistant === 'claude' || selectedAssistant === 'agent') && selectedModel === 'claude-sonnet-4') {
                ...
                <div className="ml-auto flex items-center gap-1">
                  <button
                    type="submit"
                    disabled={(!prompt.trim() && (!uploadedImages || uploadedImages.length === 0)) || isCreatingProject}
            ...
            <div className="flex flex-wrap gap-2 justify-center mt-8">
              {[
                { 
                  text: '手炒自选股用户渗透率',
                  prompt: '国内的每天增加和删除自选股的DAU分别占手炒DAU的百分比'
                },
                { 
                  text: 'AIME日活用户数统计',
                  prompt: 'AIME过去一周的平均日活跃用户数是多少？'
                },
                { 
                  text: 'Ainvest DAU趋势',
                  prompt: 'Ainvest web过去20周的DAU的趋势图'
                },
                { 
                  text: '问财新用户问句特征',
                  prompt: '过去一周问财新用户主动输入的问句都有哪些特征'
                },
                { 
                  text: 'Dreamface活跃用户',
                  prompt: '过去7天中用过5天，生成的作品数超过15个的用户有哪些人'
                }
              ].map((example) => (
                <button
                  key={example.text}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-black dark:bg-opacity-70 flex items-center justify-center z-50">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">删除项目</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400">此操作无法撤销</p>
              </div>
            </div>
            
            <p className="text-gray-700 dark:text-gray-300 mb-6">
              您确定要删除<strong>"{deleteModal.project.name}"</strong>吗? 
              这将永久删除所有项目文件和聊天记录。
            </p>
