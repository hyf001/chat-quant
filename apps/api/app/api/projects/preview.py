...
async def start_preview(
    ...
    # check project cli_preferred
    if project.preferred_cli != AgentType.NEXT_JS.value :
        html_file_name = "index.html"
        # find .html file in dashboard dir
        project_root_dir = settings.projects_root
        dashboard_dir = os.path.join(project_root_dir,project_id,"dashboard")
        files = [file for file in os.listdir(dashboard_dir) if file.endswith(".html")]
        if len(files) == 0:
            raise HTTPException(status_code=500, detail="no html file exit")
        files.sort(key=lambda x : os.path.getmtime(os.path.join(dashboard_dir,x)))
        html_file_name = files[-1]
        return PreviewStatusResponse(
                    running=True,
                    port=None,  # TODO: Store port information
                    url=f"{settings.getway_url}/api/static/{project_id}/dashboard/{html_file_name}",
                    process_id=None
                ) 
    else:
...
async def stop_preview(project_id: str, db: Session = Depends(get_db)):
    ...
    if project.preferred_cli == AgentType.ANALYIS.value:
