...
from app.services.cli import UnifiedSessionManager
...
async def get_cli_preference(project_id: str, db: Session = Depends(get_db)):
    ...
    preferred_cli = getattr(project, 'preferred_cli', 'agent')
...
async def set_cli_preference(
    ...
    # Validate CLI type
    try:
        cli_type = AgentType(body.preferred_cli)
    except ValueError:
        raise HTTPException(
            status_code=400,
...
async def get_all_cli_status(project_id: str, db: Session = Depends(get_db)):
    ...
    agent_status = await manager.check_cli_status(CLIType.AGENT)

    return AllCLIStatusResponse(
        agent=to_resp("agent", agent_status),
        preferred_cli=preferred_cli,
    )
