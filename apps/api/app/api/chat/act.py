...
from app.services.cli.manager import session_manager
...
async def execute_act_instruction(
    ...
    try:
        ...
        if not session:
            ...
            cli_type = project.preferred_cli or AgentType.NEXT_JS.value
            session = ChatSession(
                id=session_id,
        ...
        project_info = {
            ...
            'preferred_cli': project.preferred_cli or AgentType.NEXT_JS.value,
...
async def execute_chat_task(
    ...
    try:
        ...
        if cli_preference is None:
            try:
                cli_preference = AgentType(project_preferred_cli)
            except ValueError:
                ui.warning(f"Unknown CLI type '{project_preferred_cli}', falling back to Agent", "CHAT")
                cli_preference = AgentType.NEXT_JS
        ...
        cli_manager = UnifiedSessionManager(
            project_id=project_id,
    ...
    try:
        ...
        if cli_preference is None:
            try:
                ...
                ui.warning(f"Unknown CLI type '{project_preferred_cli}', falling back to Agent", "ACT")
                cli_preference = AgentType.NEXT_JS
        ...
        result = await session_manager.execute_instruction(
            session_id=session.id,
            project_id=project_id,
            instruction=instruction,
            conversation_id=conversation_id,
            cli_type=AgentType.from_value(cli_preference),
            db=db,
            images=images,
            model=project_selected_model
        )
...
async def run_act(
    ...
    cli_preference = AgentType(body.cli_preference or project.preferred_cli)
    ...
    # If no instruction but has images, use a default message
    if not message_content.strip() and body.images:
        message_content = "Analyze the uploaded image(s)"
    ...
    if image_paths:
        ...
        if message_content.strip():
            ...
            message_content = chr(10).join(image_refs)
...
@router.post("/{project_id}/interrupt", response_model=ActResponse)
async def run_interrupt(
    ...
    agent_type = AgentType.from_value(project.preferred_cli)
    await session_manager.interrupt(agent_type,body.session_id)
    
    return ActResponse(
        session_id=body.session_id,
        conversation_id="",
        status="interrupted",
        message="Chat execution interrupted"
    )
