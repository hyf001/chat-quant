...
用法:
    python3 export_datasources.py [--agent-type TYPE] [--business-lines BL1,BL2] [--dry-run] [--verbose]

示例:
    # 导出data_develop智能体类型的所有数据源
    python3 export_datasources.py --agent-type data_develop

    # 仅导出iwc业务线的数据源
    python3 export_datasources.py --agent-type data_develop --business-lines iwc

    # 预览模式（不实际写入文件）
    python3 export_datasources.py --agent-type data_develop --dry-run

    # 详细输出
    python3 export_datasources.py --agent-type data_develop --verbose
"""
import sys
import asyncio
import logging
...
async def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='导出MCP数据源到本地YAML文件')
    parser.add_argument(
        '--agent-type',
    ...
    try:
        ...
        # 初始化数据源导出器
        exporter = DataSourceExporter(config, args.agent_type)
