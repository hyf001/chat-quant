...
class DataSourceExporter(BaseExporter):
    """数据源导出器"""
    ...
    def __init__(self, config: ConfigManager, agent_type: str = "data_analysis"):
        ...
        # 获取MCP工具配置
        tool_config = get_mcp_tool_config("datasource_list")
        ...
        try:
            ...
            def update_progress(count: int, name: str):
                ...
                if verbose and (count % 10 == 0 or count <= 5):
                    logger.info(f"📊 已处理: {count} 个数据源，当前: {name}")

            # 流式处理数据源
            async for datasource in self._stream_datasource_list(update_progress):
                try:
                    # 业务线过滤
                    business_line = extract_business_line(datasource.get("business_segment", ""))
                    ...
                    yaml_data = build_datasource_yaml_data(
                        ...
                        mcp_tool="mcp__hexin-cbas-data-transmission-mcp__datasource_list"
                    )

                    # 添加文件路径信息
                    filename = sanitize_filename(datasource.get("name", f"datasource_{processed_count}"))
            ...
            logger.info("✅ 数据源导出完成!")
            logger.info(f"📊 处理数据源: {processed_count} 个")
            ...
            logger.error(f"数据源导出过程异常: {str(e)}")
            return False

        finally:
            await self.mcp_client.close()

    async def _stream_datasource_list(self,
        ...
        def internal_progress_callback(data_item: Dict[str, Any]):
            ...
            datasource_name = data_item.get("name", f"datasource_{processed_count}")

            if progress_callback:
                progress_callback(processed_count, datasource_name)

        # 使用基类的流式调用方法（使用完整的工具名称）
        async for datasource in self.mcp_client.stream_mcp_tool(
            self.tool_function,
            arguments,
